            ...
            sliver: SliverList(
              delegate: SliverChildListDelegate([
                ...
                _buildPrimaryToolCard(
                  ...
                  icon: Icons.timer,
                  title: 'Soak Timer',
                  subtitle: 'Start a soak session for your orchids',
                  color: AppTheme.waterBlue,
                  onTap: () async {
                    try {
                      final db = Provider.of<AppDatabase>(context, listen: false);
                      final started = await startStandaloneSoakWorkflow(context, db);
                      if (started && context.mounted) {
                        MainNavigation.globalKey.currentState?.switchToTab(0);
                      }
                    } catch (e) {
                      if (context.mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Something went wrong starting the soak timer')),
                        );
                      }
                    }
                  },
